...
// These mirror BuddyDomain.sol constants; bump both when the domain enum counts change.
...
function renderContract(source, centeredFrames, centeredHats) {
  ...
  lines.push("");
  lines.push('import {BuddyDomain} from "./libraries/BuddyDomain.sol";');
  lines.push("");
  ...
  lines.push("        if (");
  lines.push("            species >= BuddyDomain.SPECIES_COUNT || frame >= BuddyDomain.FRAME_COUNT");
  lines.push("                || row >= BuddyDomain.SPRITE_ROW_COUNT");
  lines.push("        ) {");
  lines.push("            revert InvalidBodyIndex();");
  lines.push("        }");
  lines.push("");
  lines.push("        uint256 rowIndex = uint256(species) * uint256(BuddyDomain.FRAME_COUNT) * uint256(BuddyDomain.SPRITE_ROW_COUNT)");
  lines.push("            + uint256(frame) * uint256(BuddyDomain.SPRITE_ROW_COUNT) + uint256(row);");
  lines.push("");
  lines.push("        return _slice(BODY_DATA, rowIndex * uint256(BuddyDomain.BODY_ROW_WIDTH), BuddyDomain.BODY_ROW_WIDTH);");
  lines.push("    }");
  lines.push("");
  lines.push("    function bodyUsesRow0(uint8 species) external pure returns (bool) {");
  lines.push("        if (species >= BuddyDomain.SPECIES_COUNT) {");
  lines.push("            revert InvalidBodyIndex();");
  lines.push("        }");
  lines.push("");
  ...
  lines.push("        if (hat >= BuddyDomain.HAT_COUNT) {");
  lines.push("            revert InvalidHatIndex();");
  lines.push("        }");
  lines.push("");
  lines.push("        return _slice(HAT_DATA, uint256(hat) * uint256(BuddyDomain.HAT_ROW_WIDTH), BuddyDomain.HAT_ROW_WIDTH);");
  lines.push("    }");
  lines.push("");
