...
async function wslWindowsHome() {
  const winProfile = process.env.USERPROFILE;
  if (winProfile) {
    const converted = winProfile.replace(/^([A-Za-z]):/, (_m, drive) => `/mnt/${drive.toLowerCase()}`).replace(/\\/g, "/");
    return converted;
  }
  try {
    ...
    for (const entry of entries) {
      if (!entry.isDirectory())
        continue;
      if (entry.name === "Public" || entry.name === "Default")
        continue;
...
async function readClaudeConfig() {
  ...
  throw new Error(`Could not read .claude.json from any candidate path:
${errors.join(`
`)}`);
}
function extractIdentity(config) {
...
function loadDeployment(chainId, dirOverride) {
  ...
  if (d.chainId !== chainId) {
    throw new Error(`deployment chainId mismatch: payload=${d.chainId} filename=${chainId} path=${path}`);
  }
  return d;
}
...
function extractCardLines(svg) {
  const f0Match = svg.match(/<g\s+id="f0"[^>]*>([\s\S]*?)<\/g>/);
  if (!f0Match || typeof f0Match.index !== "number")
    return [];
  ...
  if (railWidth === 0)
    return all;
  return all.map((l) => {
    if (railRe.test(l) || l.includes("\u2502"))
      return l;
    const trimmed = l.replace(/^\s+/, "");
    if (trimmed.length >= railWidth)
      return l;
...
var FRAME_IDS = ["f0", "f1", "f2", "fb"];
function extractSpriteFrame(svg, frameId) {
...
async function fetchTokenSvg(tokenId, net) {
  ...
  try {
    ...
    if (!image || !image.startsWith(SVG_PREFIX))
      return null;
...
function parentIsSafe(path) {
  try {
    const st = lstatSync(dirname2(path));
    return st.isDirectory() && !st.isSymbolicLink();
  } catch {
    return false;
  }
}
function targetIsSafeForRead(path, maxBytes) {
  try {
...
function targetIsSafeForWrite(path) {
  try {
...
function safeReadJson(path, validate6, maxBytes) {
  try {
    if (!parentIsSafe(path) || !targetIsSafeForRead(path, maxBytes))
      return null;
    let fd;
    let text;
    try {
    ...
    return validate6(JSON.parse(text));
  } catch {
    return null;
  }
}
function safeWriteJson(path, data, validate6) {
  let tempPath = null;
  try {
    const normalized = validate6(data);
    if (normalized === null)
      return false;
    const json = JSON.stringify(normalized);
    const dir = dirname2(path);
    mkdirSync(dir, { recursive: true });
    if (!parentIsSafe(path) || !targetIsSafeForWrite(path))
      return false;
    tempPath = join2(dir, `.json.${process.pid}.${Date.now()}.${Math.random().toString(36).slice(2)}.tmp`);
    let fd;
    try {
      fd = openSync(tempPath, fsConstants.O_WRONLY | fsConstants.O_CREAT | fsConstants.O_EXCL | O_NOFOLLOW, 384);
      writeSync(fd, json);
      try {
        fchmodSync(fd, 384);
      } catch {}
...
function pluginDataDir() {
  return join3(claudeDir(), "plugins", "buddy-onchain");
}
...
function ambientStatePath(projectDir, accountUuid, chainId) {
  return join3(pluginDataDir(), "projects", projectKey(projectDir), accountUuid, String(chainId), "state.json");
}

// src/identity.ts
function sameIdentity(a, b) {
  return a.accountUuidHash === b.accountUuidHash && a.chainId === b.chainId && a.contractAddress === b.contractAddress;
}
function identityIsUnset(identity) {
  return identity.accountUuidHash === null && identity.chainId === null && identity.contractAddress === null;
}
function currentIdentityIsLessResolvedThanCache(cached, current) {
...
// src/buddy-state.ts
class BuddyStateWriteError extends Error {
  constructor(message) {
    super(message);
    this.name = "BuddyStateWriteError";
  }
}
var VALID_MODE = new Set(["off", "lite", "full"]);
var VALID_HATCH = new Set(["unknown", "cold", "warm"]);
var MAX_STATE_BYTES = 8 * 1024;
var V4_KEYS = [
  "accountUuidHash",
  "chainId",
  "coldNudgeCounter",
...
function isMode(value) {
  return typeof value === "string" && VALID_MODE.has(value);
}
function isHatch(value) {
  return typeof value === "string" && VALID_HATCH.has(value);
}
function normalizeIdentity(raw) {
  const { accountUuidHash, chainId, contractAddress } = raw;
  if (accountUuidHash !== null && typeof accountUuidHash !== "string")
    return null;
  ...
  return {
    accountUuidHash,
    chainId,
    contractAddress: contractAddress === null ? null : contractAddress.toLowerCase()
  };
}
function validateStateV4(raw) {
  if (!isPlainObject(raw))
    return null;
  const keys = Object.keys(raw).sort();
  if (keys.length !== V4_KEYS.length)
    return null;
  if (keys.some((k, i) => k !== V4_KEYS[i]))
    return null;
  if (raw.schemaVersion !== 4)
    return null;
  if (!isMode(raw.mode) || !isHatch(raw.hatch))
    return null;
  if (typeof raw.turnCounter !== "number")
    return null;
  if (!Number.isInteger(raw.turnCounter) || raw.turnCounter < 0)
    return null;
  if (typeof raw.coldNudgeCounter !== "number")
    return null;
  if (!Number.isInteger(raw.coldNudgeCounter) || raw.coldNudgeCounter < 0)
    return null;
  ...
  if (raw.hatch === "warm") {
    if (typeof tokenId !== "string" || !/^0x[0-9a-f]+$/i.test(tokenId))
      return null;
  ...
  return {
    ...
    mode: raw.mode,
    hatch: raw.hatch,
    tokenId: tokenId === null ? null : tokenId.toLowerCase(),
...
function clearTokenIfNotWarm(state) {
  if (state.hatch === "warm")
    return state;
  return { ...state, tokenId: null };
}
function monotonicMerge(latest, next, policy) {
  const candidate = {
    ...next,
    turnCounter: Math.max(latest.turnCounter, next.turnCounter),
    coldNudgeCounter: policy.resetColdNudgeCounter ? 0 : Math.max(latest.coldNudgeCounter, next.coldNudgeCounter)
  };
  ...
  if (policy.preserveKnownHatchOnUnknown && latest.hatch === "warm" && candidate.hatch === "unknown") {
    return {
      ...candidate,
      hatch: "warm",
...
function mutateState(transform, options = {}) {
  const path = statePath();
  const base2 = readState(path) ?? defaultState();
  const policy = {
  ...
  const transformed = validateStateV4(clearTokenIfNotWarm(transform({ ...base2 })));
  if (transformed === null) {
    throw new BuddyStateWriteError("invalid buddy state transform result");
  }
  const latest = readState(path) ?? base2;
...
async function readIdentityTuple() {
  let accountUuidHash = null;
  try {
...
function getEnvMode() {
  ...
  if (!raw)
    return null;
  return VALID_MODE.has(raw) ? raw : null;
}
function derivedEveryNth(mode) {
  if (mode === "full")
    return 1;
  if (mode === "lite")
    return 3;
...
function modeFooterSentence(mode) {
  if (mode === "full")
    return "your buddy appears on every user prompt (mode: `full`).";
  if (mode === "lite")
    return "your buddy appears every 3rd prompt (mode: `lite`).";
  return "your buddy is silent on prompts (mode: `off`).";
}

// src/art-cache.ts
import { unlinkSync as unlinkSync2 } from "fs";
class BuddyArtCacheWriteError extends Error {
  constructor(message) {
    super(message);
    this.name = "BuddyArtCacheWriteError";
  }
}
var MAX_CACHE_BYTES = 32 * 1024;
var CACHE_KEYS = [
  "accountUuidHash",
  "cachedAtMs",
  "chainId",
...
function validateArtCacheV1(raw) {
  if (!isPlainObject(raw)) {
    return null;
  }
  const keys = Object.keys(raw).sort();
  ...
  if (keys.some((key, index2) => key !== CACHE_KEYS[index2])) {
    return null;
  }
...
function artCachePath() {
  return buddyArtCachePath();
}
function readArtCache() {
  return safeReadJson(artCachePath(), validateArtCacheV1, MAX_CACHE_BYTES);
}
function writeArtCache(cache) {
  if (!safeWriteJson(artCachePath(), cache, validateArtCacheV1)) {
    throw new BuddyArtCacheWriteError(`failed to write buddy art cache: ${artCachePath()}`);
  }
}
function clearArtCache() {
  try {
    unlinkSync2(artCachePath());
  } catch {}
}
function cacheMatchesIdentityAndToken(cache, identity, tokenId) {
  const token = tokenHex(tokenId);
  if (token === null) {
    return false;
  }
  return cache.accountUuidHash === identity.accountUuidHash && cache.chainId === identity.chainId && cache.contractAddress === identity.contractAddress?.toLowerCase() && cache.tokenId === token;
}
...
function validateStateFile(raw) {
  if (!isPlainObject(raw)) {
    return null;
  }
  ...
  return {
    ...
    lastFrameIndex: raw.lastFrameIndex,
    accountUuid: raw.accountUuid,
    chainId: raw.chainId
  };
}
function stateFilePath(projectDir, accountUuid, chainId) {
  return ambientStatePath(projectDir, accountUuid, chainId);
}
function readState2(path) {
  const file = safeReadJson(path, validateStateFile, MAX_STATE_BYTES2);
  if (file === null) {
    return freshState();
  }
  return {
...
function padRight3(s, width) {
  const pad4 = width - s.length;
  return pad4 > 0 ? `${s}${" ".repeat(pad4)}` : s;
}
function dedentRows(rows) {
  let start = 0;
...
function buildAdditionalContext(spriteRows, options = {}) {
  ...
  const rows = dedented.map((r, i) => {
    ...
    return ` ${padRight3(r, width)} | ${joke}`;
  });
  const block = ["```", ...rows, "```"].join(`
`);
  const directive = options.coldNudgeActive ? `${COLD_NUDGE_DIRECTIVE}
${DISPLAY_BUDDY_DIRECTIVE}` : DISPLAY_BUDDY_DIRECTIVE;
  return `${directive}

${block}`;
}
async function renderAmbientFrame(args) {
  ...
  const prev = readState2(path);
  const sameAccountAndChain = prev.accountUuid === args.accountUuid && prev.chainId === args.net.chainId;
  let baseline = prev;
  ...
  for (let attempt = 0;attempt < FRAME_IDS.length; attempt++) {
    const candidate = ((baseline.lastFrameIndex + 1 + attempt) % FRAME_IDS.length + FRAME_IDS.length) % FRAME_IDS.length;
...
function deriveBones(rng) {
  ...
  for (const s of STAT_NAMES) {
    if (s === primary) {
      stats[s] = Math.min(100, base2 + 50 + Math.floor(rng() * 30));
    } else if (s === secondary) {
      stats[s] = Math.max(1, base2 - 10 + Math.floor(rng() * 15));
    } else {
      stats[s] = base2 + Math.floor(rng() * 40);
    }
  }
...
// src/sleeping-frame.ts
var EYE_PLACEHOLDER = "0";
var BLINK_GLYPH = "-";
...
var BUDDY_COMMANDS = new Set([
  "/buddy-onchain",
  "/buddy-onchain:buddy-onchain"
]);
function isModeVerb(value) {
  return value === "off" || value === "lite" || value === "full";
}
function routePrompt(rawPrompt) {
...
// src/effective-state.ts
function deriveEffective(state, identity, envOverride) {
  return {
...
function resolveBuddyStatusMessage(args) {
...
function resultToHatch(reason) {
  if (reason === "warm-hatched")
    return "warm";
  if (reason === "cold-miss")
    return "cold";
  return "unknown";
}
function resultToChainStatus(reason) {
  if (reason === "warm-hatched" || reason === "cold-miss")
    return "online";
  return "offline";
}
...
function populateArtCacheFromSvg(svg, tokenId, identity) {
  ...
  writeArtCache({
    ...
    tokenId: tokenHex2(tokenId),
    frames,
    cachedAtMs: Date.now()
  });
}
function resetStateIdentity(identity) {
  mutateState((state) => ({
    ...state,
    ...identity,
    hatch: "unknown",
...
function invalidateMismatchedIdentity(identity) {
  const cached = readState();
  if (cached === null)
    return;
  if (identityIsUnset(cached))
    return;
  if (sameIdentity(cached, identity))
    return;
  if (currentIdentityIsLessResolvedThanCache(cached, identity))
    return;
  resetStateIdentity(identity);
}
function sleepingCardLines(accountUuid, state, effective) {
  return applySleepIndicator(sleepingFrame({ accountUuid }).rows, state, effective);
}
async function resolveAndWriteBuddyChainState(args) {
  ...
  const mappedTokenId = mappedHatch === "warm" && result.tokenId !== null ? tokenHex2(result.tokenId) : null;
  const preserveKnownHatchOnUnknown = !(args.context === "session-start" && mappedHatch === "unknown");
  const resetColdNudgeCounter = args.context === "slash" && result.reason === "cold-miss";
...
async function resolveLookupPayload(args = {}) {
  try {
    ...
    let cardLines = sleepingCardLines(canonicalUuid, result.state, effective);
    if (result.state.hatch === "warm" && result.state.tokenId !== null) {
      if (result.chainStatus === "online") {
        ...
        if (cache !== null && cacheMatchesIdentityAndToken(cache, result.identity, result.state.tokenId) && cachedRows !== undefined && cachedRows.length > 0) {
...
function pushCard(lines, payload) {
...
function formatInvalidVerbBlock(verb) {
...
// src/drift-flag.ts
import { existsSync as existsSync2, mkdirSync as mkdirSync2, unlinkSync as unlinkSync3, writeFileSync } from "fs";
...
function setDriftFlag() {
  touchFlag(driftFlagPath());
}
function clearDriftFlag() {
  clearFlag(driftFlagPath());
}
function isDriftFlagSet() {
  try {
    return existsSync2(driftFlagPath());
  } catch {
    return false;
  }
}
...
var TABLE_SEPARATOR_RE = /^\s*\|?\s*:?-{3,}/;
var MIN_BODY_ROWS = 3;
var MAX_BODY_ROWS = 8;
var MAX_NON_EMPTY_LINES_BEFORE_FENCE = 3;
function tagQualifies(tag) {
  return tag === "" || tag === "text" || tag === "txt";
}
function isClosingFence(line, fence) {
  return line.trim() === fence;
}
function detectBuddyRender(assistantText) {
...
async function readStopHookInput(timeoutMs = 200) {
...
function assistantTextFromEntry(entry) {
  ...
  const isAssistant = nestedRole === "assistant" || nestedRole === undefined && entry.type === "assistant";
  if (!isAssistant || message === null) {
    return null;
  }
  return textFromContent(message.content);
}
function assistantTextFromTranscript(path) {
...
function resolveAssistantText(input) {
...
async function processStop(input, emit2 = emitEmpty) {
  let emitted = false;
  const done = () => {
    if (emitted)
      return;
...
async function readHookInput(timeoutMs = 200) {
  if (process.stdin.isTTY)
    return null;
  return new Promise((resolve3) => {
    let buf = "";
...
function emitHookResult(driftSet, base2, expectedRender) {
  let additionalContext = base2;
  if (driftSet) {
    additionalContext = additionalContext === null ? RULESET_AMBIENT : `${RULESET_AMBIENT}
...
function tokenIdFromState(state) {
...
async function resolveAmbientAccount(args) {
  if (args.uuid) {
...
`), false);
      ...
      case "ambient": {
        ...
        let coldNudge = {
          rows: sleepRows,
          jokeOverrides: sleepRows.map(() => null)
        };
...
async function main() {
  ...
  if (args.stop && (args.hook || args.sessionStart || args.uuid !== null)) {
    console.error("buddy-onchain: --stop is mutually exclusive with --hook, --session-start, and --uuid");
    console.error("Run with --help for usage.");
    process.exit(2);
  }
  if (args.sessionStart && (args.hook || args.uuid !== null)) {
    console.error("buddy-onchain: --session-start is mutually exclusive with --hook and --uuid");
    console.error("Run with --help for usage.");
    process.exit(2);
  }
