...
function createVector(source: string, salt: string, category: VectorCategory): WyHashVector {
  ...
  const bunHash64 = BigInt.asUintN(64, Bun.hash(input));
...
function buildVectors(): WyHashVector[] {
  ...
  const robotAnchor = createVector(
    "47492784-eec5-4983-8072-9e2aa832c24b",
    DEFAULT_SALT,
    "robot-anchor"
  );
...
function buildFixture(vectors: WyHashVector[]): WyHashFixture {
  return {
    description: "WyHash Bun parity vectors over accountUuid + friend-2026-401 trait seed salt",
