...
describe("wyhash (Bun.hash)", () => {
  ...
  test("result is always a positive 32-bit integer", () => {
    const inputs = ["", "hello", "a".repeat(1000), TEST_SEED];
...
describe("computeIdentityHash", () => {
  ...
  test("matches wyhash test vector", () => {
    const hash = computeIdentityHash(TEST_UUID);
    expect(hash).toBe(WYHASH_HASH);
  });

  test("hash is always a positive 32-bit integer", () => {
    const uuids = [
    ...
    for (const uuid of uuids) {
      const hash = computeIdentityHash(uuid);
...
describe("deriveBuddyFromAccount", () => {
  test("end-to-end matches wyhash test vector", () => {
    const { identityHash, bones } = deriveBuddyFromAccount(TEST_UUID);

    expect(identityHash).toBe(WYHASH_HASH);
  ...
  test("anon fallback is deterministic", () => {
    const a = deriveBuddyFromAccount("anon");
    const b = deriveBuddyFromAccount("anon");
