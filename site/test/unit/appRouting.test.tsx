...
//   - missing/malformed `accountUuid` on `/hatch` redirects to `/`
...
// in context, which this test deliberately omits. The stub reads the route
// param via `useSearchParams` (the same react-router primitive `Hatch`
// uses internally) so the assertion `hatch placeholder for f47ac10b...`
// stays meaningful under `<MemoryRouter>`.
//
...
vi.mock('../../src/routes/Hatch', async () => {
  // Import the real module lazily so the stub can pull `useSearchParams`
  // from `react-router-dom` without colliding with `vi.mock`'s hoist.
  const { useSearchParams } = await import('react-router-dom');
  const Hatch = (): JSX.Element => {
    const [params] = useSearchParams();
    const uuid = params.get('accountUuid') ?? '';
    return <main data-testid="hatch-stub">hatch placeholder for {uuid}</main>;
  };
  return { Hatch };
});
