...
`plugin/src/instructions.ts::STATUSLINE_NUDGE_TEMPLATE(command)` appends to `SessionStart` output when no badge heartbeat exists — neither this project's nor the global one (see § Badge heartbeat; `settings.json` is never probed — a `statusLine` key proves nothing about the badge actually rendering). The template carries the full platform-matched command from `plugin-paths.ts::statuslineCommand()` — `bash` + `buddy-statusline.sh`, or `powershell` + `buddy-statusline.ps1` on Windows — and instructs the model to compose (not replace) when a custom statusline already exists. The command points at the version-stable script copy in the plugin data dir (`statusline-install.ts` refreshes it at SessionStart and on every slash lookup), never the version-pinned marketplace cache path (stale/dead after a plugin update) and never a `${CLAUDE_PLUGIN_ROOT}` literal (does not interpolate at statusline runtime).
...
- Slash lookup checks `hasProjectBadgeHeartbeat` only — per-project, so a badge rendering in another open session cannot mask a project whose `.claude/settings.json` statusline shadows the badge. Never created → `LookupPayload.statuslineWireHint` carries the full platform command for the stable data-dir script copy and `formatLookupBlock` appends a `statusline:` + `setup:` line pair to the card (cold and warm alike — the badge renders unhatched too; phrasing is declarative so the rendering model treats it as display text, not instructions).
