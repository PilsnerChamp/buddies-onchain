...
- **Deterministic traits, not random.** The contract stores a caller-supplied seed — the plugin computes it from your account UUID client-side — and derives traits from that seed via Mulberry32. The chain proves consistency: the stored traits match the stored seed, and anyone can recompute `traits == _deriveTraits(seed)`. It does not prove the seed came from your account; that link is established off-chain. Anyone who knows your UUID can precompute the same seed and the same traits before you hatch. This is by design for reproducible hatching; it is not a secrecy or randomness guarantee.
...
- **A squatter can hatch your identity hash with their own seed.** `hatch()` is permissionless and the identity hash is visible before a transaction confirms, so anyone who knows a victim's identity hash can hatch it first — and because the contract now derives traits from a caller-supplied seed, the squatter can supply any seed they like and choose, or grind for, the body and traits on that token. This body-choice capability is new: it did not exist when the contract derived the body from identity. It stays harmless, and harmless only because buddies are valueless. The squatted token is held by the contract, owned by no one, and unbondable — bonding (dormant) re-checks both the identity hash and the seed against a signed attestation, and a foreign seed fails that check. The result is a dead record, not a hijack: the squatter cannot take, sell, or bond it, and the real-UUID holder is not permanently locked out. Recovery is the same supersede path bonding provides — denied until the Stage-2 supersede move lets the rightful holder reclaim, never permanently denied. The denial itself is not new; permissionless mint-once has always allowed squat-denial. What is new is only the cosmetic body-choice, and it confers nothing because the token has no value to confer.
